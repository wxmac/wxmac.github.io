...
### new Map()
```javascript
//  <button @click="onClickEvent">触发</button>
...
mounted() { 
    setTimeout(() => {
      const resolve = this.events.get('test')
      resolve && resolve('数据')
    }, 10000)
},
methods: {
    async onClickEvent() {
      const data = await this.sleepEvent()
      console.error('onClickEvent->', data) // 点击， 10s后打印出 数据
    },
    sleepEvent() {
      return new Promise((resolve) => {
        this.events.set('test', resolve)
      })
    },
},
```
    ...
    function getParams (name) {
        ...
        if (window.URLSearchParams) {
            ...
            for (const param of params) {
                ...
                if (pair[0] === name) {
                    // URL-decode the parameter value and return
...
function init(obj, name, opt) {
  console.error('obj, name, opt--->', obj, name, opt) // {} id 11
  obj[name] = opt
}
...
```javascript
newTextContent(item) {
    const httptext = item.content
    const reg = /(http:\/\/|https:\/\/)((\w|=|\?|\.|\/|&|-)+)/g
    const urlText = httptext.replace(reg, "<a style='color: #2440b3' target='__blank' href='$1$2'>$1$2</a>")
    // console.error('urlText', urlText)
    return urlText
}
```
### 顺滑滚动
...
onSetImgListScroll(type) {
    let scrollAmount = 0
    const listEl = this.$refs.imgList;
    const scrollMax = listEl.scrollWidth
    const scrollMin = 0
    const result = type === 'right' ?  Math.max(scrollAmount += this.max, scrollMax) : Math.min(scrollAmount -= this.max, scrollMin);
...
### 输入最多保留两位小数
```javascript
 <el-input v-model="purchaseAmount" max="10" @input="purchaseAmount = purchaseAmount.match(/\d+(\.\d{0,2})?/) ? purchaseAmount.match(/\d+(\.\d{0,2})?/)[0] : ''" type="number" style="width:100px;margin:0 20px 0 0"></el-input>
...
// instance--> Test1 {instance: null, name: '1111'} Test1 {instance: null, name: '1111'} Test1 {instance: null, name: '2222'} true
```

### 按传入顺序返回列表
```javascript
const listId = [ 1, 2, 3, 9 ]
const list = [
    { id: 1 },
    { id: 3 },
    { id: 2 },
...
const newList = listId.map((item) => list.find((k) => k.id === item))
...
let parentIndex = arr1.length;

while (parentIndex--) {
	console.log(parentIndex + '=' + arr[parentIndex]);

	if (arr1[parentIndex].checked) {
		arr1.splice(parentIndex, 1);
	}

    if (arr1[parentIndex].data) {
        let childIndex = arr1[parentIndex].data.length;
        while (childIndex--) {
            if (arr1[parentIndex].data[childIndex].checked) {
                arr1[parentIndex].data.splice(childIndex, 1);
            }
        }
    }
...
### 设置鼠标滚轮默认滚动方式为横向
```js
    const el = parentTableRef.value
    el.addEventListener('wheel', (event) => {
        const { scrollLeft, scrollWidth, clientWidth } = el;
        const { deltaY } = event;
        const maxScrollLeft = scrollWidth - clientWidth;
        const left = el.scrollLeft + event.deltaY;
