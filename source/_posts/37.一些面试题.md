...
```javascript
// 1. 创建一个继承构造函数原型的对象 Object.create(Super.prototype);
// 2. 构造函数的方法和属性设置到 对象上，并执行  Super.apply(obj,arg); 
// 3. 执行结果有返回值并且是一个对象，返回执行的结果，否侧返回新创建的对象 result instanceof Object ? result : obj;

// 手写new
function New(Super, ...args) {
    // 创建一个继承构造函数原型的对象/ 将空对象的原型proto指向构造函数的原型prototype
    let obj = Object.create(Super.prototype);
    // 将this指向新创建的对象，并且执行构造函数
    const result = Super.apply(obj,arg);
...
isSymmetryNum(1, 100); // 11 22 33 44 55 66 77 88 99
...
*          协商缓存 etag   Last-Modify/If-Modify-Since 和 ETag/If-None-Match.
 * tcp 三次握手
 * http请求服务器
 * 服务器响应
*          根据 HTML 生成 dom树
